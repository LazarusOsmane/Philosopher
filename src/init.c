...
/*   Updated: 2022/06/16 01:17:59 by engooh           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "../inc/philo.h"
...
{
	while (++i < data->nbp_std)
		if (pthread_create(&data->philo[i].thrid, NULL, routine, &data->philo[i]) < 0)
			return (0);
	return (1);
}
...
int	init_philo(t_data *data, int ac, char **av)
{
	...
	if (!philo)
		return (0);
	data->philo = philo;
	if (!set_data_philo(data, -1) || !create_mutex(data, -1) || !create_thread(data, -1))
		return (0);
	return (1);
}
